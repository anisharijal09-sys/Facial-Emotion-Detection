...
FEATURE_LEN = 180
TARGET_SR = 48000  # ✅ RAVDESS was recorded at 48kHz
...
print("Audio classes:", le.classes_)
...
def extract_feature(y, sr):
    # ✅ Resample to 48000Hz to match RAVDESS training data
    if sr != TARGET_SR:
...
def predict(data: ImageData):
    ...
    try:
        ...
        # ✅ Try FER first - detects ALL faces
        if fer_detector is not None:
            try:
                ...
                if result and len(result) > 0:
                    ...
                    if boxes_list:
                        print(f"✅ FER DETECTED {len(boxes_list)} face(s)")
                        return {
                            "emotion": boxes_list[0]["label"],
                            "boxes": boxes_list
                        }
        ...
        if image_model is not None:
            ...
            if hasattr(results[0], "boxes") and results[0].boxes is not None:
                ...
                if len(boxes_list) > 0:
                    ...
                    print(f"✅ YOLO DETECTED {len(boxes_list)} face(s), Top: {top_emotion}")
                    return {
                        "emotion": top_emotion,
                        "boxes": boxes_list
                    }
...
async def predict_audio(chunk: AudioChunk):
    ...
    for i, score in enumerate(preds):
        print(f"  {le.classes_[i]}: {score:.3f}")
    print("--------------------")
    ...
    # ✅ Map labels to match image model
    if emotion == "calm":
        emotion = "neutral"
    ...
    print(f"✅ AUDIO FINAL: {emotion} ({confidence:.2%})")
